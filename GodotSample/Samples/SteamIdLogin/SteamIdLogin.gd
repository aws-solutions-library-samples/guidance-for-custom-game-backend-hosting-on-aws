...
func _ready():
	...
	aws_games_sdk_auth.steam_link.connect(on_link_steam_id_response)
	aws_games_sdk_auth.steam_login.connect(on_login_with_steam_id_response)
...
func _on_login_success():
	...
	aws_games_sdk_auth.link_steam_id_to_current_user("YourTokenHere")
...
func on_link_steam_id_response():
	...
	# NOTE: You need to use the Godot Steamworks SDK (https://godotsteam.com/) to integrate with Steam
	#       Use the GetAuthTicketForWebAPI to get the steam auth token (https://godotsteam.com/functions/users/#getauthticketforwebapi)
	aws_games_sdk_auth.login_with_steam_token("YourTokenHere")

	
func on_login_with_steam_id_response():
