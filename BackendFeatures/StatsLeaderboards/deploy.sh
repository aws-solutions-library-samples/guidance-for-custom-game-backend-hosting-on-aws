...
echo "  • 11 Lambda functions with shared layer"
...
echo "   Game ID: $GAME_ID"
echo ""
echo "2. Verify your studio/game registration:"
echo "   Your studio and first game were registered AUTOMATICALLY during this"
echo "   deployment, using the details in studio_parameters.json. If you did not"
echo "   edit that file first, those details are STAND-IN PLACEHOLDERS and should"
echo "   be corrected to your real studio/game before going further."
echo ""
echo "   Registered now as:"
echo "     Studio Name:  $STUDIO_NAME"
echo "     Game Title:   $GAME_TITLE"
echo "     Game Genre:   $GAME_GENRE"
echo "     Contact:      $CONTACT_EMAIL"
# Detect the shipped placeholder values and nudge the user to update them.
if [ "$STUDIO_NAME" = "Example Game Studio LLP" ] || [ "$GAME_TITLE" = "Example Speedinators V" ] || [ "$CONTACT_EMAIL" = "contact@example.com" ]; then
    echo ""
    echo "   ⚠️  These look like the SHIPPED PLACEHOLDER values. Update them to your"
    echo "      real studio/game with the developer-registration API below (the API"
    echo "      key, Studio ID, and Game ID above stay the same)."
fi
echo ""
echo "   Check what is registered:"
echo "   curl -X GET \"${API_ENDPOINT}developer/info?studioId=$STUDIO_ID&gameId=$GAME_ID\" \\"
echo "     -H \"Authorization: Bearer $API_KEY\""
echo ""
echo "   Update your studio/game details (if the above are placeholders or wrong):"
...
echo ""
echo "   Until you do this, player endpoints return HTTP 401 with integration"
echo "   guidance (and the comprehensive test's player-auth phases will fail)."
echo ""
echo "   To integrate:"
...
echo "     -H \"Authorization: Bearer $API_KEY\""
echo ""
echo "5. Run the comprehensive integration + smoke test:"
echo "   A single suite (testing/test_StatsAndLeaderboards.py) auto-discovers this"
echo "   stack's endpoint and API key, exercises every endpoint end-to-end, and"
...
echo "       player-facing test phases fail with HTTP 401 by design."
echo "     • Add --retain to keep created leaderboards for manual inspection."
echo ""
echo "6. View the system documentation:"
echo "   cat readme.md                 # overview + integration guide"
echo "   cat docs/api_reference.md     # full API reference (Section 1.5 = player auth)"
echo ""
echo "7. Check deployment status anytime:"
echo "   ./deployment_info.sh"
echo ""
echo "📋 Important Notes:"
echo "==================="
echo "• This deployment infrastructure has been set up for $STUDIO_NAME's - $GAME_TITLE"
echo "• Studio/game registration was done automatically during deploy from studio_parameters.json"
echo "  (verify/correct it — step 2 — if you didn't edit that file before deploying)"
echo "• Player-facing APIs deny all requests until you integrate player auth (step 3)"
...
echo "• The Studio API Key is available in CloudFormation outputs and SSM Parameter Store"
echo "• The Studio API Key is for BACKEND/server use only — never embed it in game clients"
echo ""
